...
6. Add `RESEND_API_KEY` and `EMAIL_FROM` for registration and order email delivery. The store owner recipient is configured as `gauravsharma2000gk@gmail.com`. For production, verify your sending domain in Resend and use an address on that domain for `EMAIL_FROM`.
